...
const async_to_array_1 = __importDefault(require("iter-tools/methods/async-to-array"));
...
async function* allSolutions(...assertions) {
...
async function prove(conjecture) {
...
async function solve(conjecture) {
    const solver = new conjecture.ctx.Solver();
...
describe('high-level', () => {
    let api;
    beforeAll(async () => {
        api = await (0, jest_1.init)();
    });
    afterAll(async () => {
        await (0, jest_1.killThreads)(api.em);
    });
    it('can set params', () => {
    ...
    it('checks that Context matches', () => {
        ...
        expect(() => c1.Or(c2.Int.val(5).eq(2))).toThrowError(types_1.Z3AssertionError);
        // On the other hand, this won't compile due to automatic generics
        // @ts-expect-error
        expect(() => c3.Or(c4.Int.val(5).eq(2))).toThrowError(types_1.Z3AssertionError);
    ...
    describe('ints', () => {
        it('finds a model', async () => {
            ...
            (0, assert_1.default)(isIntVal(xValueExpr));
            (0, assert_1.default)(isIntVal(yValueExpr));
            const xValue = xValueExpr.value();
            const yValue = yValueExpr.value();
            (0, assert_1.default)(typeof xValue === 'bigint');
            (0, assert_1.default)(typeof yValue === 'bigint');
        ...
        it('solves sudoku', async () => {
            function toSudoku(data) {
                const cells = Array.from({ length: 9 }, () => Array.from({ length: 9 }, () => null));
      ...
      `);
            ...
            for (let i = 0; i < 9; i++) {
                ...
                for (let j = 0; j < 9; j++) {
                    ...
                    (0, assert_1.default)(isIntVal(cell));
                    const value = cell.value();
                    (0, assert_1.default)(typeof value === 'bigint');
    ...
    describe('bitvectors', () => {
        it('can do simple proofs', async () => {
            ...
            (0, assert_1.default)(isBitVecVal(sSol) && isBitVecVal(uSol));
        ...
        it('finds x and y such that: x ^ y - 103 == x * y', async () => {
            ...
            (0, assert_1.default)(isBitVecVal(xSol) && isBitVecVal(ySol));
    ...
    describe('arrays', () => {
        ...
        it('domain and range type inference', async () => {
            ...
            (0, assert_1.default)(isArray(arr) && isArraySort(arr.sort));
            ...
            (0, ts_expect_1.expectType)(dom2.size());
        ...
        it('can do simple proofs', async () => {
            ...
            (0, assert_1.default)(isArray(arr) && isArraySort(arr.sort) && isConstArray(constArr));
        ...
        it('Array type inference', async () => {
            ...
            const Z3_GLOBAL_STORAGE = z3.Array.const('global_storage', z3.BitVec.sort(160), z3.Array.sort(z3.BitVec.sort(160), z3.BitVec.sort(256)));
    ...
    describe('quantifiers', () => {
        ...
        it('Loading Quantifier Preserves Type', async () => {
            ...
            const quantifier2 = solver2.assertions().get(0);
    ...
    describe('uninterpreted functions', () => {
        it('Type Inference', async () => {
            ...
            (0, ts_expect_1.expectType)(output);
    ...
    describe('Solver', () => {
        ...
        it('can find multiple solutions', async () => {
            ...
            const x = Int.const('x');
            const solutions = await (0, async_to_array_1.default)(allSolutions(x.ge(1), x.le(5)));
            expect(solutions.length).toStrictEqual(5);
            const results = solutions
                .map(solution => {
                const expr = solution.eval(x);
                (0, assert_1.default)(isIntVal(expr));
                return expr.value();
            })
                .sort((a, b) => {
                (0, assert_1.default)(a !== null && b !== null && typeof a === 'bigint' && typeof b === 'bigint');
                if (a < b) {
                    return -1;
                }
                else if (a == b) {
